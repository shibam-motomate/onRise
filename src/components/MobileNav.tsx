...
export function MobileNav() {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 250 }}>
      ...
      <div
        style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: 284, maxWidth: '82vw', background: '#463A2A', boxShadow: '20px 0 60px rgba(20,28,22,.45)', padding: '24px 22px', display: 'flex', flexDirection: 'column', gap: 2, animation: 'deoOverlay .25s ease both' }}
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <BrandLogo size="sm" onDark mark={false} />
        ...
        {links.map((m) => (
          <div key={m.label} onClick={m.go} className="hv-row" style={{ cursor: 'pointer', padding: '15px 12px', borderRadius: 10, fontSize: 16, color: '#EDE7D8', borderBottom: '1px solid rgba(255,255,255,.07)' }}>
            {m.label}
          </div>
        ))}
      </div>
